...
- **Tutorial scenes** that teach each new spell before next opponent
...
* As the user progresses through the campaign the spells shown unlocked and the new spells get dynamically added.
...
Each map node has a `spellReward` field. Win the fight, earn the spell. This made implementation easier for me since for each location I could simply add the spellReward to each location isntead of having to wire componenets together over and over agian creating new tutorial scences teaching the user how to use each new unlocked spell.
...
Unlock scenes sometimes uses a `TutorialCrystal` instead of a live opponent. The crystal is a simple `IDamageable` that calls `TutorialManager.CompleteCurrentStep()` when destroyed — no health bar, no AI, no pressure. Some steps use the enemy with attacks disabled for more seemless graphical experience.
