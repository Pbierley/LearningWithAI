...
- Solutions
  - To handle the first issue we used AI to code detection settings that allowed for multiple frames to be captured with generosity that allowed for "missed frames" while holding up a set of gestures
    - This significantly improved the issues with various hands, and even improved detection in poor lighting
  - To better improve detection in poor lighting, AI gave us a few ideas including:
    - Include a white ring around the game
    - Prompt users to adjust their environment for better lighting
    - And the already implemented generous code for discarding botched frames
  - From that we decided that the white ring around the screen could be a nice feature that could be toggled on and off but the results from it were minimal and the negative UI experience we determined made this not worthwhile.
...
- Got help wiring the gesture classifier output into Unity's game loop — see [Map User Flow](MapUserFlow.md)
  - Initially we used Unity's AI for this and it was very fast but made many mistakes — I still found it the best solution for speedy development wiring Unity GameObjects to scripts.
  - Since Unity AI was broken in a later release and would cause the editor to frequently crash, we had to uninstall the package and move to using AI in VSCode / Cursor editors
  - This took more time because unlike Unity's AI, we had to describe the context of what was happening in the editor and what GameObjects' attributes were.
- Used AI to troubleshoot timing issues between gesture detection and spell casting
  - It took a lot of refinement to determine the best number of frames a hand gesture should be captured before adding it to the queue.
...
- Claude explained how polycount affects render performance and walked me through:
  ...
  - Decimating and simplifying meshes without visibly degrading art quality
- These optimizations restored smooth framerate without sacrificing the look of the scene

![SceneImage](SceneImage.png)
...
- This currently causes some weird things to occur such as the opponent healing whenever they're already at full health, dodging "phantom" attacks, and never intentionally doing counter attacks such as the attack where the enemy levitates and throws mana balls (which would dodge the current attack coming towards it and counter with several mana balls coming towards the user)
...
- AI helped me think through what the enemy should be "rewarded" for (e.g. dealing damage, surviving, successfully countering a spell)
  - Some of these are implemented more easily than others such as:
    - If the user is sending an attack then the CPU should attempt to queue a shield or dodge.
    - If no attack is queued from the user then the CPU should attempt to heal if health is not full
    - Else queue an attack
  - More complicated implementations are:
    - Trying to program the CPU to successfully time a counter attack like the levitation described previously

---

![FlowChart](EnemyAttackFlowChart.png)
...
- AI gave examples of how to tune parameters to adjust the difficulty of the enemy
- Used AI to understand how to prevent the enemy from becoming too predictable or too random
    - Parameters:
        - CPU presets:
            ...
            - Spell Cooldown
            - CPU Unlocked Spells
            - Spell Attack Damages
        - Adjustable counters:
            - What % of attacks are countered
            - When it counters, how often is it just a defense vs. a counter attack (counter attack being more difficult)
...
- Built and trained an RL-based enemy AI that reacts dynamically to player spell usage
